  ...
  try {
    ...
    if (searchType === 'all' || searchType === 'prayers') {
      try {
        ...
        // First try text search on multiple fields
        let prayersQuery = supabase
          ...
          .select(`
            ...
            user_id
          `)
          .or(`request.ilike.%${searchQuery}%,name.ilike.%${searchQuery}%,category.ilike.%${searchQuery}%`)
    ...
    if (searchType === 'all' || searchType === 'journal') {
      try {
        ...
        // Search multiple fields in journal
